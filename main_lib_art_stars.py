...
def detection(im_name):
    ...
    iraf.daofind(image=im_name + '[0]', threshold=3.00, output='default', verbose='no', verify='no')
    iraf.pdump(infile=im_name + '0.coo.1', fields="XCENTER, YCENTER, MAG, SHARPNESS, SROUND, GROUND", expr="(MAG != INDEF)&&(SHARPNESS != INDEF)&&(SROUND != INDEF)&&(GROUND != INDEF)", Stdout= im_name+ '_det.dat')
    iraf.pdump(infile=im_name + '0.coo.1', fields="ID, XCENTER, YCENTER, MAG, SHARPNESS, SROUND, GROUND", expr="(MAG != INDEF)&&(SHARPNESS != INDEF)&&(SROUND != INDEF)&&(GROUND != INDEF)", Stdout= im_name+ '_det.dat_with_n')
    print('Detection run on {}'.format(im_name))
...
def ap_phot(im_name, coord_file, max_n_psf):
    ...
    iraf.phot(image=im_name+'[0]', coord=coord_file, output='default', verbose='no', verify='no')
    iraf.pdump(infile=im_name + '0.mag.1', fields="ID,XCENTER,YCENTER,MAG,MERR,SHARPNESS,CHI", expr="(MAG != INDEF)&&(MERR != INDEF)", Stdout=im_name + '0.mag')
    #iraf.pstselect(image=im_name+'[0]', photfile='default', pstfile='default', maxnpsf=max_n_psf, verbose='no', verify='no')
...
def write_ZP(infile, infile_DES, band, mag_lim_sat_DES):
    ...
    cond = (MAG_DES > mag_lim_sat_DES + 1.)&(MAG_DES < mag_lim_sat_DES + 3.)
...
def create_pst_based_on_multipar(phot_file, mag_low, band, pst_file, nmax_psf, INIT_ANG_DIST, MAX_MSKY):
    ...
    ID, XCENTER, YCENTER, SHARPNESS, SROUND, GROUND, MAG = np.loadtxt(phot_file, usecols=(0, 1, 2, 4, 5, 6, 10), unpack=True)
    ...
    fig, (ax0, ax1, ax2, ax3) = plt.subplots(nrows=1, ncols=4, sharey=False, figsize=(18, 6))
    ax0.hist(SHARPNESS, bins=30, range=(np.min(SHARPNESS), np.max(SHARPNESS)), histtype='step', label='Median={:.3f}'.format(median_sharp))
    ax0.set_xlabel('Sharpness')
    ax0.set_ylabel('counts')
    ax0.grid()
    ax0.legend()
    ax1.hist(SROUND, bins=30, range=(np.min(SROUND), np.max(SROUND)), histtype='step', label='Mean={:.3f}'.format(mean_sround))
    ax1.set_xlabel('Sround')
    ax1.grid()
    ax1.legend()
    ax2.hist(GROUND, bins=30, range=(np.min(GROUND), np.max(GROUND)), histtype='step', label='Mean={:.3f}'.format(mean_ground))
    ax2.set_xlabel('ground')
    ax2.grid()
    ax2.legend()
    ax3.hist(MAG, bins=30, range=(np.min(MAG), np.max(MAG)), histtype='step', label='Mag')
    ax3.set_xlabel('Mag')
    ax3.legend()
    ax3.set_yscale('log')
    ...
    plt.hist(sharp_norm, bins=30, range=(-4, 4), histtype='step', label='sharp_norm', color='b')
    plt.hist(sround_norm, bins=30, range=(-4, 4), histtype='step', label='sround_norm', color='g')
    plt.hist(ground_norm, bins=30, range=(-4, 4), histtype='step', label='ground_norm', color='r')
    plt.hist(newpar, bins=30, range=(-4, 4), histtype='step', label='new_par', color='maroon')
    plt.legend()
    ...
    XCENTER, YCENTER, ID, SHARPNESS, SROUND, GROUND, MAG, sharp_norm, sround_norm, ground_norm, newpar = XCENTER[bright_star], YCENTER[bright_star], ID[bright_star], SHARPNESS[bright_star], SROUND[bright_star], GROUND[bright_star], MAG[bright_star], sharp_norm[bright_star], sround_norm[bright_star], ground_norm[bright_star], newpar[bright_star]
    ...
    for i in idx_par_sort:
        print(int(ID[i]), XCENTER[i], YCENTER[i], sround_norm[i], ground_norm[i], newpar[i], file=f)
    f.close()
        ...
        if count < nmax_psf:
            if idx_par_sort[i] in idx_pst:
                ...
                if msky[idx_] < MAX_MSKY:
                    print("{:<9d}{:<10.3f}{:<10.3f}{:<12.3f}{:<15.7f}".format(idx_pst[idx_], xcenter[idx_], ycenter[idx_], mag[idx_], msky[idx_]), file=g)
                    count += 1
        else:
            break
    g.close()
...
def PSF_phot(fits_image):
    ...
    iraf.allstar(image=fits_image+"[0]", photfile='default', psfimage= tilename + '.fits0.psf.1.fits', cache='no', allstarfile='default', rejfile='default',subimage='default', verify='no', verbose='no') #psfimage='default',
...
def wcs(im_name, infile, outfile):
    iraf.wcsctran(input=infile, output=outfile, image=im_name+'[0]', inwcs='logical', outwcs='world', col="2 3")
...
bands = ['g', 'r', 'i']
files = sorted(glob.glob('*.fits0.add.*.fits'))
...
#parte do codigo para rodar nas imagens com estrelas adicionadas
for ii in files:
    for jj in ['i', 'g']: 
        tilename = ii[0:13] + jj
        print(tilename) 
        for k in range(2,11):
            ...
            image_name = glob.glob(ii[0:13] + jj + '.fits0.add.'+ str(k) +'.fits')[0]
        
            detection(image_name)
            print('running photometry on', image_name)
            det_file, coo_file = image_name + '_det.dat', image_name + '0.coo.1'  
            print(det_file)
            phot_pdump_file = image_name + '0.mag'
        
            ap_phot(image_name, det_file, 200)
               
            os.system('sort -k 1b,1 ' + phot_pdump_file + ' > ' + phot_pdump_file + '_sorted')
            os.system('sort -k 1b,1 ' + det_file + '_with_n > ' + det_file + '_sorted')
