...
def write_ZP(infile):
    ...
    image_DES = '../1_DES_images_and_cats/images/' + infile[:-16] + 'fits.fz'
    infile_DES = '../1_DES_images_and_cats/cats/' + infile[:-18] + 'gold_y6.fits'
    band = infile[13:14]
    ...
    EXT_DES = hdu[1].data.field('ext_coadd')
    MAG_DES = hdu[1].data.field('wavg_mag_psf_'+ band)
    hdu.close()
    
    cond = (MAG_DES > mag_lim_sat_DES[band] + 1.)&(MAG_DES < mag_lim_sat_DES[band] + 3.)&(EXT_DES == 0)
    ...
    plt.scatter(MAG_DES[idx_des], MAG_DAO[idx_dao]-ZP, label='data', s=0.2)
    plt.plot(xx, xx, color='k', label='Fit')
    plt.xlabel('MAG in {} band'.format(band))
    plt.legend()
    plt.xlabel('Magnitude in DES')
    plt.ylabel('Magnitude in DAO')
    plt.title('Best-Fitting for {} band: {:.4f}'.format(band,
              ZP))
    plt.legend(loc=2)
    plt.xlim([np.min(MAG_DAO[idx_dao]) - 0.5 - ZP, np.max(MAG_DAO[idx_dao]) + .5 - ZP])
    plt.ylim([np.min(MAG_DAO[idx_dao]) - 0.5 - ZP, np.max(MAG_DAO[idx_dao]) + .5 - ZP])
    plt.savefig(infile[:-18] + band +'_ZP' + '.png')
    plt.close()

    print(infile, band, -1*ZP)
    
    g = open(infile[:-16] + '_calibrated_with_wcs.dat', 'w')

    for i in range(len(IDX)):
        print(IDX[i], X_DAO[i], Y_DAO[i], RA_DAO[i], DEC_DAO[i], MAG_DAO[i] - ZP, MAG_ERR[i], SHARPNESS[i], CHI[i], file=g)
    g.close()

pre_cal_cat = glob.glob('*pre_cal.dat')
