...
def detection(im_name):
    ...
    iraf.pdump(infile=im_name + '0.coo.1', fields="XCENTER, YCENTER, MAG, SHARPNESS, SROUND, GROUND",
               expr="(MAG != INDEF)&&(SHARPNESS != INDEF)&&(SROUND != INDEF)&&(GROUND != INDEF)", Stdout=im_name[:12] + '_det.dat')
    iraf.pdump(infile=im_name + '0.coo.1', fields="ID, XCENTER, YCENTER, MAG, SHARPNESS, SROUND, GROUND",
               expr="(MAG != INDEF)&&(SHARPNESS != INDEF)&&(SROUND != INDEF)&&(GROUND != INDEF)", Stdout=im_name[:12] + '_det.dat_with_n')
...
def PSF_phot(fits_image, imp_pst_file):
    ...
    iraf.psf(image=fits_image + "[0]", photfile='default', pstfile=imp_pst_file, psfimage='default',
             opstfile='default', groupfile='default', interactive='no', verify='no', verbose='no')
    ...
    iraf.allstar(image=fits_image+"[0]", photfile='default', psfimage='default', cache='no', allstarfile='default',
                 rejfile='default', subimage='default', verify='no', verbose='no')  # psfimage='default',
...
tiles = np.unique([i[0:13] for i in det_images])
bands = ['g', 'r', 'i', 'z', 'Y']
...
for ii in det_images:
    ...
    for jj in bands:
        ...
        det_file = image_name[:12] + '_det.dat'
        ...
        os.system('sort -k 1b,1 ' + phot_pdump_file +
                  ' > ' + phot_pdump_file + '_sorted')
        os.system('sort -k 1b,1 ' + det_file +
                  '_with_n > ' + det_file + '_sorted')
        os.system('join ' + det_file + '_sorted ' + phot_pdump_file +
                  '_sorted > ' + image_name + '_parsfile.dat')

        imp_pst_file = create_pst_based_on_multipar(
            image_name + '_parsfile.dat', 2.0, jj, pst_file, 50, 20, 100)
